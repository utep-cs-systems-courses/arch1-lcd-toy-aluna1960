...
void fillTriangle(u_char colMin, u_char rowMin,u_int colorBGR){
  for (int r = 0; r < 50; r++){
    for(int c = 0; c < r; c++){
      drawPixel(c,r,COLOR_PURPLE);
      drawPixel(c-r, r, colorBGR);
    }
  }
}
...
/*void drawChar9x7(u_char rcol, u_char rrow, char c, 
     u_int fgColorBGR, u_int bgColorBGR) 
{
  u_char col = 0;
  u_char row = 0;
  ...
  u_int colorBGR = (font_9x7[1] & bit) ? fgColorBGR : bgColorBGR;
  lcd_writeColor(colorBGR);
...
void drawChar11x16(u_char rcol, u_char rrow, char c, char afta, 
     u_int fgColorBGR, u_int bgColorBGR) 
{
  u_char col = 0;
  u_char row = 0;
  ...
  u_char oc = c - 0x20;
  //u_char aft = afta-0x20;
  lcd_setArea(rcol, rrow, rcol + 10, rrow + 15); /* relative to requested col/row */
  while (row < 16) {
    while (col < 11) {
      /*if(row > 8){
	u_int colorBGR = (font_11x16[aft][col] & bit) ? fgColorBGR : bgColorBGR;
	lcd_writeColor(colorBGR);
	col++;
	}*/
      //else{
